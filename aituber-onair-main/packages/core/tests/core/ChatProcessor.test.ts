...
describe('ChatProcessor', () => {
  ...
  describe('processTextChat', () => {
    ...
    it('should regenerate before publishing when provider reports truncation', async () => {
      mockChatService.chatOnce
      ...
      expect(emitSpy).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          truncated: false,
          finishReason: 'stop',
          screenplay: expect.objectContaining({ text: 'Complete response.' }),
        }),
      );
