...
describe('AITuberOnAirCore processing flow', () => {
  ...
  it('applies the final response gate before emitting and speaking', async () => {
    ...
    const chatProcessor = (core as any).chatProcessor;
    const voice = vi.mocked(VoiceEngineAdapter).mock.results.at(-1)?.value as {
      speak: ReturnType<typeof vi.fn>;
    };
    let emittedText = '';
    core.on(AITuberOnAirCoreEvent.ASSISTANT_RESPONSE, (payload: any) => {
      emittedText = payload.screenplay.text;
    });

    chatProcessor.emit('assistantResponse', {
      message: { role: 'assistant', content: '污染回答[e~[' },
      screenplay: { text: '污染回答[e~[', ttsText: '污染回答[e~[' },
    });
    await Promise.resolve();

    expect(emittedText).toBe('已清洗回答');
    expect(voice.speak).toHaveBeenCalledWith(
      expect.objectContaining({ text: '已清洗回答', ttsText: '已清洗回答' }),
