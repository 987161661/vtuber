...
export interface AITuberOnAirCoreOptions {
  ...
  /**
   * Final synchronous gate applied after model generation and before the
   * response event, history consumers, and TTS. Throwing from the gate fails
...
export class AITuberOnAirCore extends EventEmitter {
  ...
  /** Speech decisions are consumed in the same order as LLM responses. */
  private readonly responseSpeechQueue: boolean[] = [];
  ...
  private setupEventForwarding(): void {
    ...
    this.chatProcessor.on('assistantResponse', async (data) => {
      ...
      // The live operator may deliberately decline a redundant message. This
      // sentinel is an internal queue decision, never audience-facing speech.
      if (screenplay.text?.trim() === '[[NO_REPLY]]') return;

      const shouldSpeak = this.responseSpeechQueue.shift() ?? true;
