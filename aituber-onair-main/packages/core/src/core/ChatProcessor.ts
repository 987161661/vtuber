...
export class ChatProcessor extends EventEmitter {
  ...
  async processTextChat(
    ...
    try {
      ...
      const cleanTransientContext = transientContext?.trim();
      if (cleanTransientContext) {
        // Context is available to this generation only. It must never be
        // copied into chatLog, otherwise live transcripts and skill payloads
        // multiply on every subsequent turn.
        initialMsgs.splice(Math.max(0, initialMsgs.length - 1), 0, {
          role: 'system',
          content: cleanTransientContext,
        });
      }
    ...
    while (hops++ < this.maxHops) {
      ...
      if (stop_reason === 'end') {
        ...
        if (truncated) {
          ...
          throw new Error(
            'Assistant response remained truncated after continuation retries',
          );
        }
        ...
        const assistantMessage: Message = {
          ...
          // out of the next-turn conversational context.
