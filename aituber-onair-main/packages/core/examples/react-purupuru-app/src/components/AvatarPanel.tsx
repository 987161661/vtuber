...
export function AvatarBackground({
  ...
  const [displayedSpeakingUrl, setDisplayedSpeakingUrl] = useState<
    string | null
  >(speakingAvatarVideoUrl);
  ...
  useEffect(() => {
    if (speakingAvatarVideoUrl) {
      const frame = requestAnimationFrame(() => {
        setDisplayedSpeakingUrl(speakingAvatarVideoUrl);
        setSpeakingLayerVisible(true);
      });
      return () => cancelAnimationFrame(frame);
    }
    const frame = requestAnimationFrame(() => setSpeakingLayerVisible(false));
    const timer = window.setTimeout(() => setDisplayedSpeakingUrl(null), 180);
    return () => {
