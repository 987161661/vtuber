...
const GRIEF =
  /(?:去世|走了|离世|死了|没了|永远离开|葬礼|火化|猫|狗|宠物).{0,8}(?:去世|走了|离世|死了|没了)?/u;
...
function hasRecentGrief(turns: RecentLiveTurn[]): boolean {
  return turns
    .slice(-4)
    .some((turn) => GRIEF.test(`${turn.input} ${turn.reply ?? ''}`));
}
...
const SCENES = new Set<InteractionScene>([
  ...
  'correction', 'advice_rejection', 'question', 'boundary',
...
const MOVES = new Set<SocialMove>([
  ...
  'clarify', 'correct_self', 'set_boundary', 'deescalate', 'welcome',
  'invite_room',
]);
...
export function formatPersonaInteractionPlan(
  ...
  const lines = [
    '<persona_interaction>',
    room
      ? `房间简报：本批${room.totalCount}条、${room.participantCount}人；车道=${Object.entries(room.laneCounts).map(([lane, count]) => `${lane}:${count}`).join(',').slice(0, 120) || 'conversation'}；冲突=${room.conflictLevel}。只回应已选代表弹幕，不逐条复述。`
      : '',
