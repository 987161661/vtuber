...
export interface MinimaxAudioSettingsOptions {
  /** Sampling rate in Hz (MiniMax defaults to 44100) */
  sampleRate?: number;
...
interface VoiceServiceBaseOptions {
  ...
  /** Streaming audio playback callback. */
  onPlayStream?: (
...
export interface MinimaxVoiceServiceOptions extends VoiceServiceCommonOptions {
  /** Engine type */
  engineType: 'minimax';
  /** Optional same-origin MiniMax gateway URL for browser runtimes. */
  ...
  /** Use MiniMax HTTP streaming output for earlier audio playback. */
