...
import { sanitizeSpeechText } from '../utils/sanitizeSpeechText';
import {
  AudioPlayOptions,
  VoiceService,
  VoiceServiceOptions,
...
type PreparedSpeech =
  ...
  | {
      kind: 'audioStream';
      audioStream: AsyncGenerator<ArrayBuffer>;
    }
  | {
      kind: 'direct';
...
export class VoiceEngineAdapter implements VoiceService {
  ...
  private async prepareSpeechForScreenplay(
    ...
    const message = sanitizeSpeechText(screenplay.ttsText ?? screenplay.text);
    if (!message) return { kind: 'silent' };
    ...
    if (
      this.options.engineType === 'minimax' &&
      this.options.minimaxStream !== false &&
      'fetchAudioStream' in engine &&
      typeof engine.fetchAudioStream === 'function'
    ) {
      return {
        kind: 'audioStream',
        audioStream: engine.fetchAudioStream(
          talk,
  ...
  private async playPreparedSpeech(
    ...
    if (preparedSpeech.kind === 'silent') return;
    ...
    if (preparedSpeech.kind === 'audioStream') {
      if (this.options.onPlayStream) {
        await this.options.onPlayStream(preparedSpeech.audioStream, options);
        return;
      }
      for await (const audioBuffer of preparedSpeech.audioStream) {
