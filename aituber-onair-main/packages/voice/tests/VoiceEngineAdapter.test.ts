...
describe('VoiceEngineAdapter', () => {
  ...
  describe('MiniMax Integration', () => {
    ...
    it('should allow MiniMax synthesis when GroupId is not provided', async () => {
  ...
  describe('Text Processing', () => {
    ...
    it('should never synthesize model-internal control blocks', async () => {
      const onPlay = vi.fn();
      const adapter = new VoiceEngineAdapter({
        engineType: 'minimax',
        speaker: 'test-speaker',
      ...
      await adapter.speakText(`<audience_low_engagement>
新进来的观众可能还在观望。
</audience_low_engagement>`);

      expect(mockEngine.fetchAudio).not.toHaveBeenCalled();
      expect(onPlay).not.toHaveBeenCalled();
    });
  });
