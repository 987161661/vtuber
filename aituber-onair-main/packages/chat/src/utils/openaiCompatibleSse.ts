...
type SseParseOptions = {
  ...
  /** Maximum lifetime of a response stream after headers arrive. */
  totalTimeoutMs?: number;
};

const DEFAULT_STREAM_IDLE_TIMEOUT_MS = 30_000;
const DEFAULT_STREAM_TOTAL_TIMEOUT_MS = 90_000;
...
const forEachSsePayload = async (
  ...
  options: Pick<SseParseOptions, 'idleTimeoutMs' | 'totalTimeoutMs'> = {},
): Promise<void> => {
  ...
  const totalTimeoutMs = Math.max(
    idleTimeoutMs,
    options.totalTimeoutMs ?? DEFAULT_STREAM_TOTAL_TIMEOUT_MS,
  );
  ...
  try {
    while (!shouldStop) {
      ...
      if (remainingTotalMs <= 0) {
        throw new Error(`Response stream timeout after ${totalTimeoutMs}ms`);
      }
      ...
      const timeout = new Promise<never>((_, reject) => {
        timeoutId = setTimeout(
          () =>
            reject(
              new Error(`Response stream idle timeout after ${timeoutMs}ms`),
            ),
          timeoutMs,
        );
      });
      const read = await Promise.race([reader.read(), timeout]).finally(() => {
